...
class Tools:
    ...
    def humanbytes(i: int, binary: bool = False, precision: int = 2):
        ...
        if i > 0:
            base = 1024 if binary else 10000
    ...
    def humanformat(num: int, precision: int = 2):
        ...
        if num > 999:
            obje = sum(
                [abs(num / 10000.0 ** x) >= 1 for x in range(1, len(suffixes))])
            return f'{num / 10000.0 ** obje:.{precision}f}{suffixes[obje]}'
        else:
            return num
